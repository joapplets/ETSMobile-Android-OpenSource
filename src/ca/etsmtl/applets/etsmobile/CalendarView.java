...
public class CalendarView extends ImageView 
{
    ...
    private Cell mSelectedDay = null;
	...
	private void initCells() {
	    ...
	    int thisDay = 0;
	    mToday = null;
	    if(mHelper.getYear()==mRightNow.get(Calendar.YEAR) && mHelper.getMonth()==mRightNow.get(Calendar.MONTH)) {
		...
		for(int week=0; week<mCells.length; week++) {
			for(int day=0; day<mCells[week].length; day++) {
				...
				if(mSelectedDay != null) {
					mDecoration.setBounds(mSelectedDay.getBound());
				}
    ...
    public void nextMonth() {
    	mSelectedDay =  mCells[0][0];
    	mHelper.nextMonth();
    	Log.v("mRightNow", "Year:"+mRightNow.get(Calendar.YEAR)+"  month:" +mRightNow.get(Calendar.MONTH));
    	initCells();
    	invalidate();
    }
	...
	//added by dom
	public void refreshToSelectedDate(Cell cell) {
		mSelectedDay = cell;
		if(mDecoration!=null && mSelectedDay!=null) {
