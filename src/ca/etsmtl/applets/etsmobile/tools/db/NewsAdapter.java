...
public class NewsAdapter {
	...
	public static NewsAdapter getInstance(final Context c) {

		// if instance is null, which means the we are opening the app
		if (NewsAdapter.instance == null) {

			// create one instance
			NewsAdapter.instance = new NewsAdapter();

			// create the db, if it's already in the filesystem it will not
			// recreate it
			NewsAdapter.db = new SQLDBHelper(c, NewsAdapter.DB_NAME, null,
					NewsAdapter.DB_VERSION);
			NewsAdapter.db.close();
		}

		// return the instance.
		return NewsAdapter.instance;
	}
	...
	public ArrayList<News> getAllNews() {
		...
		final SQLiteDatabase readDB = NewsAdapter.db.getReadableDatabase();
	...
	public ArrayList<News> getNewsBySource(final ArrayList<String> source) {
		...
		final Cursor c = readDB.query(NEWS_TABLE, new String[] { "*" },
				sources, null, null, null, NEWS_DATE + " DESC");
	...
	public void insertNews(final String title, final long date,
		...
		NewsAdapter.db.getWritableDatabase().insert(NEWS_TABLE, null, cv);
