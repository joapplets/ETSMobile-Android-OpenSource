...
public class XMLRssFbTwitterHandler extends XMLNewsHandler{
	...
	public XMLRssFbTwitterHandler(Context context, String source) {
		super(context);
		this.source = source;
	}
	...
	public void endElement(String uri, String localName, String qName) throws SAXException {
		...
		if(localName.equalsIgnoreCase(TITLE) && inItem){
			...
			// On reinitialise le buffer à nouveau pour ne pas avoir des titres dans la 
		...
		// Quand on arrive à la fin d'un champs item, on ajoute la "currentNews" dans la liste
		...
		if(localName.equalsIgnoreCase("item") & inItem){
			...
			try {
				Date date = dateFormat.parse(currentNewsDate);
