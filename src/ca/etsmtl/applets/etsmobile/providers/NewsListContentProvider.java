...
public class NewsListContentProvider extends android.content.ContentProvider {
	...
	private static final String AUTHORITY = "ca.etsmtl.applets.etsmobile.data.news";
	...
	public synchronized Cursor query(Uri uri, String[] columns,
		...
		case ALL_NEWS:
			...
			break;
		case SINGLE_NEWS:
			queryBuilder.appendWhere(NewsTable.NEWS_ID + "="
		...
		if (sURIMatcher.match(uri) == ALL_NEWS) {
			...
			try {
				if (writable.isOpen()) {
					...
					writable.insert(NewsTable.TABLE_NAME, null, values);
	...
	private synchronized void verifyColumnExists(String[] columns) {
		String[] available = { NewsTable.NEWS_DATE, NewsTable.NEWS_TITLE,
				NewsTable.NEWS_DESCRIPTION, NewsTable.NEWS_GUID,
				NewsTable.NEWS_ID, NewsTable.NEWS_SOURCE, NewsTable.NEWS_LINK };
		if (columns != null) {
