...
public class BottinService extends Service implements Observer {
	...
	protected Void doInBackground(final Void... arg0) {
		...
		final String request = "<?xml version=\"1.0\" encoding=\"utf-8\"?>"
			...
			+ "<soap:Body>" + "<Recherche xmlns=\"http://etsmtl.ca/\">"
			+ "<FiltreNom></FiltreNom>" + "<FiltrePrenom></FiltrePrenom>"
			+ "<FiltreServiceCode></FiltreServiceCode>" + "</Recherche>"
			+ "</soap:Body>" + "</soap:Envelope>";
		final URL url = new URL("http://etsmtl.ca/cmspages/webservice.asmx?op=Recherche");
		final HttpURLConnection conn = (HttpURLConnection) url.openConnection();
		conn.setDoOutput(true);
		conn.setRequestMethod("POST");
		conn.setRequestProperty("Host", "etsmtl.ca");
		conn.setRequestProperty("Content-Type", "text/xml; charset=utf-8");
		conn.setRequestProperty("SOAPAction", "\"http://etsmtl.ca/Recherche\"");
