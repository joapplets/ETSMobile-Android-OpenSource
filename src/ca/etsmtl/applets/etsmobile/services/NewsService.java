	...
	private class Fetcher extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(final Void... params) {
			try {
				...
				XMLNewsParser newsParser = new XMLNewsParser(
				...
				stream = url.openStream();
				newsParser = new XMLNewsParser(NewsService.FACEBOOK, guids,
						bundle);
				saxParser.parse(stream, newsParser);
				stream.close();
				...
				stream = url.openStream();
				newsParser = new XMLNewsParser(NewsService.TWITTER, guids,
						bundle);
				saxParser.parse(stream, newsParser);
				stream.close();
				...
				stream = url.openStream();
				newsParser = new XMLNewsParser(NewsService.INTERFACE, guids,
						bundle);
				saxParser.parse(stream, newsParser);
				stream.close();
