...
public class NewsService extends Service implements Observer{
	...
	private class Fetcher extends AsyncTask<Void, Void, Void>{
		...
		protected Void doInBackground(Void... params) {
			try {
				...
				Cursor cursor = getContentResolver().query(ETSMobileContentProvider.CONTENT_URI, projection, null, rss, null);
				...
				cursor = getContentResolver().query(ETSMobileContentProvider.CONTENT_URI, projection, null, fb, null);
				...
				cursor = getContentResolver().query(ETSMobileContentProvider.CONTENT_URI, projection, null, tw, null);
				guids = new ArrayList<String>();
				if(cursor.moveToFirst()){
					do{
						guids.add(cursor.getString(cursor.getColumnIndex(NewsTableHelper.NEWS_GUID)));
	...
	private void insertNewsIntoDB(News n){
		...
		getContentResolver().insert(ETSMobileContentProvider.CONTENT_URI, n.getContentValues());
