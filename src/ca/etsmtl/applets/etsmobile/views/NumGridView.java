...
public class NumGridView extends View implements Observer{
	...
	protected CalendarCell[][]    mCells;      // Contains number to be shown in each grid cell
     ...
     */
    public void setCell(int x, int y, CalendarCell v) {
     ...
     */
    public CalendarCell getCell(int x, int y ) {
        ...
        for( int y=0; y<mCellCountY; y++ ) {
            for( int x=0; x<mCellCountX; x++ ) {
              ...
              cell=  mCells[x][y];
                ...
                else if(cell.getDate().getMonth() == this.current.getTime().getMonth())
                {
                	this.mPaintFg.setAlpha(100);
                ...
                // Draw the cell value                
                int v= cell.getDate().getDate();
                canvas.drawText( ""+v, dx+tx, dy+ty, mPaintFg );
                
               Iterator<CalendarEvent> it = cell.iterator();
               CalendarEvent event;
               
               int i=0;
               
               float radius = (mCellWidth/((3*this.maxIndicators)+1));
               float startpos = (float) (dx+tx) - ((((2 * cell.size() + (cell.size()-1)) * radius)/2) - radius);
