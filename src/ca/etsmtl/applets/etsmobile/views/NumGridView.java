	...
	protected void onDraw(final Canvas canvas) {
		...
		for (int y = 0; y < mCellCountY; y++) {
			for (int x = 0; x < mCellCountX; x++) {
				...
				final Iterator<CalendarEvent> it = cell.iterator();
				CalendarEvent event;
				...
				while (it.hasNext()) {
					...
					canvas.drawCircle(startpos + 3 * i * radius, dy
							+ mCellHeight - 2 * radius, radius,
							mPaintCourseIndicator);
					i++;
				}

				// mPaintCourseIndicator.setColor(mCells[x][y].get)
				// // Test code to draw ascender stick and descender stick
				// Paint p = new Paint();
				// p.setColor(0xFFff0000);
				// canvas.drawRect( new Rect(dx+tx,dy+ty,
				// dx+tx-8,dy+ty+(int)mPaintFg.ascent()), p);
				// canvas.drawRect( new Rect(dx+tx,dy+ty,
				// dx+tx+8,dy+ty+(int)mPaintFg.descent()), p);
			}
		}
	}
		...
		for (int y = 0; y < mCellCountY; y++) {
			for (int x = 0; x < mCellCountX; x++) {
				...
				if (mCells[x][y].getDate().getDate() == 1
					...
					// set test data
					mCells[x][y].add(new CalendarEvent("LOG720", "Cours",
							"A-1424", Color.RED, mCells[x][y].getDate(),
							mCells[x][y].getDate()));
					mCells[x][y].add(new CalendarEvent("LOG619", "Cours",
							"A-1234", Color.YELLOW, mCells[x][y].getDate(),
							mCells[x][y].getDate()));
					mCells[x][y].add(new CalendarEvent("LOG720", "TP",
							"A-4321", Color.GREEN, mCells[x][y].getDate(),
							mCells[x][y].getDate()));
					mCells[x][y].add(new CalendarEvent("LOG121", "TP",
							"A-2222", Color.CYAN, mCells[x][y].getDate(),
							mCells[x][y].getDate()));
					mCells[x][y].add(new CalendarEvent("LOG210", "TP",
							"A-1111", Color.MAGENTA, mCells[x][y].getDate(),
							mCells[x][y].getDate()));
					mCells[x][y].add(new CalendarEvent("LOG210", "TP",
							"A-1111", Color.GRAY, mCells[x][y].getDate(),
							mCells[x][y].getDate()));
					mCells[x][y].add(new CalendarEvent("LOG210", "TP",
							"A-1111", Color.BLACK, mCells[x][y].getDate(),
							mCells[x][y].getDate()));
				}
