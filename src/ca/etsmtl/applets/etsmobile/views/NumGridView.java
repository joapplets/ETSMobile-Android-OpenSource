...
public class NumGridView extends View implements Observer {
	...
	final int nbCellulesY = 6; // on va g�n�rer toujours un grilles 7 x 6
	...
	public void update(final Observable observable, final Object data) {
		...
		// Possibilit� d'avoir 2 sessions dans 1 mois
			...
			for (int x = 0; x < mCellCountX; x++) {
				...
				for (final Session s : sessions) {
					...
					if (mCells[x][y].getDate().after(s.getDateDebut())
						...
						for (final JoursRemplaces j : s.getJoursRemplaces()) {
							...
							if (mCells[x][y].getDate().getDate() == j
									...
									&& mCells[x][y].getDate().getMonth() == j
											.getDateRemplacement().getMonth()
									&& mCells[x][y].getDate().getYear() == j
											.getDateRemplacement().getYear()) {
								if (s.getActivities(String.valueOf(j
										.getDateOrigine().getDay())) != null) {
									mCells[x][y]
											.setEvents(s.getActivities(String
													.valueOf(j.getDateOrigine()
															.getDay())));
								} else {
									mCells[x][y].clear();
								}
						...
						if (!isRemplacee
								&& s.getActivities(String.valueOf(mCells[x][y]
										.getDate().getDay())) != null) {
							mCells[x][y].setEvents(s.getActivities(String
									.valueOf(mCells[x][y].getDate().getDay())));
						}
