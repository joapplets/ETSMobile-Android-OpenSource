package ca.etsmlt.applets.etsmobile.utils;
...
public class ETSNotificationManager {
	...
	public static void showNotification(Context context, String title, String subject , Intent intent, int id){
		
		long when = 2000; 
		final KeyguardManager keyGuardManager = (KeyguardManager) context
				.getSystemService(Context.KEYGUARD_SERVICE);
		final boolean isScreenOff = keyGuardManager.inKeyguardRestrictedInputMode();
		...
		if (currentapiVersion >= android.os.Build.VERSION_CODES.HONEYCOMB){
			...
			Notification notif = new Notification(R.drawable.icon, subject , when);
			notif.defaults |= Notification.DEFAULT_SOUND;
			notif.defaults |= Notification.DEFAULT_VIBRATE;
			notificationManager.notify(id, notif);
		}
